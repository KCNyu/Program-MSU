...
double gold(double fx(double, double, double alpha), double x, double y)
{
    ...
    f_lambda= fx(x,y,lambda); f_mu = fx(x,y,mu);
    for (int i = 0; fabs(mu- lambda) >epsilon; i++)
    {
        if (f_lambda < f_mu)
        {
            b_gss = mu; mu = lambda; f_mu = f_lambda;
            lambda = a_gss + (1.0 - tau)*(b_gss - a_gss);
            f_lambda = fx(x,y,lambda);
        }
        else
        {
            a_gss = lambda; lambda = mu; f_lambda = f_mu;
...

{
    ...
    for (float i = 0.1; i <= 10; i += 0.5)
    {
        ...
        for (float x=-5;x<= 5;x+=0.001)
            glVertex2f(x/5,y1(x,i)/5);
        glEnd();
        
        glBegin(GL_LINE_STRIP);
        glColor3f(1,0,0);
        for (float x=-5;x<= 5;x+=0.001)
            glVertex2f(x/5,y2(x,i)/5);
        glEnd();
    }
    glFlush();          //更新窗口
