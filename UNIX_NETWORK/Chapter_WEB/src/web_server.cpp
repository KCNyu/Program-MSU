...
void WebServer::SendError(int cfd, int status, const char *title,
        const char *text) {
    char buf[4096] = {0};
    ...
    sprintf(buf + strlen(buf),
            "<body bgcolor=\"#87CEEB\"><h2 align=\"center\">%d %s</h4>\n", status,
            title);
    sprintf(buf + strlen(buf), "%s\n", text);
    sprintf(buf + strlen(buf), "<hr>\n</body>\n</html>\n");
    send(cfd, buf, strlen(buf), 0);
}
void WebServer::SendRespondHead(int cfd, int no, const char *desp,
