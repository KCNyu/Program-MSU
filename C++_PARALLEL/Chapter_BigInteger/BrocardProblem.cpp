...
BigInteger Factorial(int n)
{
	...
	transform(std::execution::par_unseq,
		  ...
		  [&bis](pair<int, int> &elem)
		  { return BigInteger(elem.first).pow(elem.second); });

	BigInteger r = reduce(std::execution::par_unseq, bis.begin() + 1, bis.end(), bis[0], [](auto fir, auto sec)
			      { return fir * sec; });
	return r;
}
